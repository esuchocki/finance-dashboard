...
const PatternsAndAlerts: React.FC<PatternsAndAlertsProps> = ({ transactions, intercompanyTransactions = [] }) => {
  ...
  const [selectedDuplicate, setSelectedDuplicate] = React.useState<{ transactions: BusinessTransaction[], reason: string, confidence: string } | null>(null);
  ...
  const handleDuplicateClick = (group: { transactions: BusinessTransaction[], reason: string, confidence: string }) => {
    setSelectedDuplicate(group);
    setViewMode('duplicate-detail');
  };
    ...
    return (
      <Card>
        ...
        <CardContent>
          ...
          <div className="max-h-[700px] overflow-y-auto space-y-6 pr-2">
            {/* Summary */}
            <Alert className={confidenceColors[group.confidence as 'high' | 'medium' | 'low']}>
